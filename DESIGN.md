...
Color strategy: _Restrained_ - deep neutrals carry the surface, one warm brass accent does all
...
- **Focus-pull** (the signature): on each new word, `#phrase-container` resolves from
  ...
  `#target` uses the same blur→crisp settle. This is the _only_ choreographed motion.
