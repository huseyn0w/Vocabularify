...
- **928 are "pure" cross-level duplicates** - the German word (`word_2`) is _identical_ across every level it appears in. These are true duplicates: the same German word is being taught at 2–4 different levels.
  - **264 of these appear in 3+ levels.**
  - Removing them down to a single level would delete **~1193 redundant entries**.
- **584 cross-level words have a _different_ German value per level** - these are partly legitimate (different sense per level) and partly noise (article/synonym variants like `orange` → `orange`/`die Orange`, `great` → `klasse`/`super`/`toll`). Review case-by-case; do not bulk-delete.
